...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    private void notifyWatchFace() {
        Uri uri = WeatherContract.WeatherEntry.buildWeatherLocationWithStartDate(
                        Utility.getPreferredLocation(getContext()), System.currentTimeMillis());

        Cursor cursor = getContext().getContentResolver()
                .query(uri, NOTIFY_WEATHER_PROJECTION, null, null, null);
        ...
        GoogleApiClient apiClient = new GoogleApiClient.Builder(getContext()).addApi(Wearable.API).build();
        ConnectionResult connectionResult = apiClient.blockingConnect(60, TimeUnit.SECONDS);

        if (!connectionResult.isSuccess())
            return;

        PutDataMapRequest weatherRequest = PutDataMapRequest.create(WEATHER_INFO_PATH);
        DataMap dataMap = weatherRequest.getDataMap();

        dataMap.putString(KEY_UUID, UUID.randomUUID().toString());
        ...
        PutDataRequest request = weatherRequest.asPutDataRequest();
        
	Log.d(LOG_TAG, "Sending weather data");
        Log.d(LOG_TAG, "MAX: " + cursor.getDouble(INDEX_MAX_TEMP) + ", MIN: "
                + cursor.getDouble(INDEX_MIN_TEMP) + ", ID: " + cursor.getInt(INDEX_WEATHER_ID));
        ...
        Wearable.DataApi.putDataItem(apiClient, request).setResultCallback(new ResultCallback<DataApi.DataItemResult>() {
            ...
            public void onResult(DataApi.DataItemResult dataItemResult) {
                if (!dataItemResult.getStatus().isSuccess())
                    Log.d(LOG_TAG, "[Fail] Weather data");
                else
                    Log.d(LOG_TAG, "[Success] Weather data");
            }
        });
    }
